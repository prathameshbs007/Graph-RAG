...
def retrieve_context(query_text: str):
    ...
    # 2. Qdrant Top-K
    search_results = db.search_all_classes(query_vector, clip_vector=clip_vector, limit=settings.RETRIEVAL_TOP_K)

    if not search_results:
        return [], [], {"related_papers": [], "concepts": []}

    # Extract unique paper IDs
    paper_ids = list(set([res["paper_id"] for res in search_results if res.get("paper_id")]))

    # 3. Neo4j Graph Traversal
    graph_context = graph_db.get_related_graph_context(paper_ids)

    # 4. Rerank: Only rerank text/audio chunks. Protect figures.
    text_results = [res for res in search_results if res["modality"] != "image"]
    image_results = [res for res in search_results if res["modality"] == "image"]

    docs_for_rerank = [res.get("chunk_text", "") for res in text_results]

    top_indices = reranker.rerank(query_text, docs_for_rerank, top_n=settings.RERANK_TOP_N)
