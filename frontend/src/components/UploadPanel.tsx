...
type FileType = 'pdf' | 'audio' | 'image' | null;

function detectFileType(file: File | null): FileType {
    if (!file) return null;
    if (file.type === 'application/pdf' || file.name.match(/\.pdf$/i)) return 'pdf';
    if (file.type.startsWith('audio/') || file.name.match(/\.(mp3|wav|m4a|ogg|flac|aac|wma)$/i)) return 'audio';
    if (file.type.startsWith('image/') || file.name.match(/\.(jpg|jpeg|png|gif|bmp|webp|tiff|svg)$/i)) return 'image';
    return null;
}
...
export const UploadPanel = () => {
    ...
    const handleUpload = async (e: React.FormEvent) => {
        ...
        let endpoint = '/ingest/pdf';

        if (fileType === 'audio') {
            endpoint = '/ingest/audio';
    ...
    const fileTypeLabel = fileType === 'pdf' ? 'PDF' : fileType === 'audio' ? 'Audio' : fileType === 'image' ? 'Image' : null;

    return (
        <div className="p-6 bg-white border rounded-xl max-w-xl mx-auto shadow-sm">
            <h2 className="text-2xl font-semibold mb-6">Ingest Document (PDF / Audio / Image)</h2>
